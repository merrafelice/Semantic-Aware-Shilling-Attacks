...
from application.parsers.run_mul import parse_run_topological, print_args
from application.relatedeness.bin.rel_utils.io_util import save_obj
from application.relatedeness.bin.rel_utils.relatedness import evaluate_katz_relatedness, evaluate_exclusivity_relatedness
from application.relatedeness.bin.rel_utils.timer import timer
...
def build():
    args = parse_run_topological()
    print_args(args)

    topk = args.topk  # The threshold on which we build the relatedness measures
    topn = args.topn  # The number of extracted top shortest paths (Not useful in Katz)
    top_k_similar_items = args.top_k_similar_items

    # Parameters
    alpha = args.alpha

    # Which measures?
    is_katz = 1 if args.is_katz == 1 else 0
    is_exclusivity = 1 if args.is_exclusivity == 1 else 0
