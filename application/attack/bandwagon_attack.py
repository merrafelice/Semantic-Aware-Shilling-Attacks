...
class BandWagonAttack(IAttack):

    def __init__(self, dataframe, r_max, r_min):
        super(BandWagonAttack, self).__init__(dataframe, r_max, r_min)
        self.fillerSize = self.get_filler_size()
        self.selectedSize = self.get_selected_size()
        self.attackSize = self.get_attack_size()
        self.bandwagonAttackType = cfg.bandwagonAttackType  # 1 = Average, 0 = Random
        self.popular_items = list(
            self.dataframe.groupby('itemId').size().reset_index(name='count').sort_values(by='count', ascending=False)[
                'itemId'])[:10]
        ...
        for i in range(self.attackSize):
            ...
            for selected_item_id in selected_items:
                shilling_profiles = shilling_profiles.append({
                    ...
                    'rating': self.targetRating  # MAX for Push and min for Nuke
                }, ignore_index=True)
            ...
            for filler_item_id in filler_items:
                shilling_profiles = shilling_profiles.append({
                    'userId': start_shilling_user_id,
                    'itemId': int(filler_item_id),
                    'rating': self.clamp(int(np.random.normal(
                        self.dataframe[self.dataframe.itemId == filler_item_id].rating.mean(),
                        self.dataframe[self.dataframe.itemId == filler_item_id].rating.std(ddof=0), 1).round()[
                                                 0])) if self.bandwagonAttackType
                    else self.clamp(int(np.random.normal(self.datasetMean, self.datasetStd, 1).round()[0]))
                }, ignore_index=True)
    ...
    def get_filler_size(self):
        """
        |I_{F}|=\frac{(\frac{\sum_{u \in U} | I_{u |}}{|U|})}{2}
        :return: Filler Size
        """
    ...
    def get_selected_size(self):
        """
        |I_{S}|=\frac{(\frac{\sum_{u \in U} | I_{u |}}{|U|})}{2} - 1
        :return: Selected Size
        """
        selectedsize = int((self.dataframe.shape[0] / self.dataframe.userId.nunique()) / 2 - 1)
        return selectedsize
    ...
    def get_filler_items(self, selectedItems, target_item_id):
        ...
        if cfg.semantic:
            ...
            # Here is the main core
            elif cfg.semantic_attack_type == cfg.attack_target_similar:
                df_similar_items = pd.read_csv(os.path.join(self.project_dir, cfg.data, cfg.dataset, cfg.similarities,
                                                            sim_file_name))
                possible_filler_items = literal_eval(
                    df_similar_items[df_similar_items['itemId'] == target_item_id]['similar_items'].values[0])
    ...
    def get_selected_items(self, target_item_id):
        ...
        popular_list = list(
            self.dataframe[self.dataframe.itemId != target_item_id].groupby(['itemId']).size().reset_index(
                name='counts').sort_values(by=['counts'], ascending=False).itemId)
        return popular_list[:self.selectedSize] if self.target else popular_list[
                                                                    (len(popular_list) - self.selectedSize):]
