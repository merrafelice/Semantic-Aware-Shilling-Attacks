...
class LoveHateAttack(IAttack):

    def __init__(self, dataframe, r_max, r_min):
        super(LoveHateAttack, self).__init__(dataframe, r_max, r_min)
    ...
    def generate_profile(self, target_item_id, sample):
        ...
        for i in range(self.attackSize):
            ...
            # In this case we remove the most similar and use the less similar ones (1-cfg.top_k_similar_items)*100 items
            ...
            for filler_item_id in filler_items:
                shilling_profiles = shilling_profiles.append({
                    ...
                    'rating': cfg.r_min if self.target else cfg.r_max  # Target = 1 is PUSH, Otherwise is 0 NUKE
                }, ignore_index=True)
    ...
    def get_filler_items(self, selectedItems, target_item_id):
        ...
        if cfg.semantic:
            ...
            if cfg.semantic_attack_type == cfg.attack_popular_similar:
                ...
                possible_filler_items = list(
                    self.dataframe[~self.dataframe['itemId'].isin(possible_filler_items)]['itemId'].unique())
