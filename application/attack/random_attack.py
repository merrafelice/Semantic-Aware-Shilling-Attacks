...
class RandomAttack(IAttack):
    ...
    def __init__(self, dataframe, r_max, r_min):
        super(RandomAttack, self).__init__(dataframe, r_max, r_min)
        ...
        for i in range(self.attackSize):
            ...
            for filler_item_id in filler_items:
                shilling_profiles = shilling_profiles.append({
                    ...
                    'rating': self.clamp(int(np.random.normal(self.datasetMean, self.datasetStd, 1).round()[0]))
                }, ignore_index=True)
        ...
        if cfg.semantic:
            ...
            if cfg.semantic_attack_type == cfg.attack_popular_similar:
                ...
                possible_filler_items = literal_eval(
                    df_similar_items[df_similar_items['itemId'] == np.random.choice(self.popular_items, 1, replace=False)[0]]['similar_items'].values[0])
